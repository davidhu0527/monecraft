...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Worldgen determinism | `lib/world/generation.test.ts`                                                                                                                     | SHA-256 digests of generated worlds per seed (the save-compat contract), plus structural probes, meshing snapshots, opaque/glass layer boundaries, and thin door geometry                                       |
...
| Inventory algebra    | `lib/game/inventory.test.ts`                                                                                                                       | Stack math, durable-item non-stacking, durability, crafting (including refuse-on-full), armor equip rules                                                                                                       |
| Save format          | `lib/game/save.test.ts`                                                                                                                            | Round-trips, legacy-shape parsing and durable-stack splitting, corrupt-data rejection                                                                                                                           |
| Simulation           | `lib/game/engine/GameEngine.test.ts`                                                                                                               | Headless gameplay: boot, movement, hunger/regen, water, mining, doors, spear reach/throws, commands, death/respawn, night spawning, saves, and events                                                           |
...
| Renderer logic       | `lib/game/render/{mobVisuals,spearVisuals,heldItem,heldItemPose,extrudedSprite,itemModel,playerModel,playerPose,playerVisuals,cameraView}.test.ts` | Model lifecycle (including thrown spears), positioning, bob, item swaps, held-item poses, sprite extrusion, player-body poses/visibility, and third-person camera placement — pure Three.js, no DOM             |
...
| Browser E2E          | `e2e/smoke.e2e.ts`                                                                                                                                 | Real browser: boot without console errors, scene draws, input → movement, craft via UI, mining, camera view cycling, skin pick persisting across reload, pause freeze/resume, save via pause menu across reload |
