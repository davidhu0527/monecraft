...
- The one binary asset is the UI pixel font (Pixelify Sans), loaded in `app/fonts.ts` via `next/font/google` (self-hosted at build) and exposed as `--mc-font`; the monospace stack stays as the fallback and on the F3 debug readout. This is the single sanctioned exception to "zero binary assets".
